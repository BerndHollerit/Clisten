...
module ClrClassInstanceMethods
  ...
  def import(name, &block)
    member_info = get_instance_member_info(Generate::to_camel_case(name.to_s), name.to_s)
...
module ClrClassStaticMethods
  ...
  def import(name, &block)
    static_member_info = get_static_member_info(Generate::to_camel_case(name.to_s), name.to_s)
