...
namespace RubyClr {
  ...
  public:
    Generator(VALUE self) {
      if (self != Qnil) {
        _generator  = (ILGenerator^)Marshal::ToObject(rb_funcall(self, rb_intern("generator"), 0));
        _namespaces = (ArrayList^)Marshal::ToObject(rb_funcall(self, rb_intern("namespaces"), 0));
      }
    }
  ...
  ref class RubyType {
    ...
    Type^ GetMemberType(RubyMemberInfo^ members) {
      Type^ baseMemberType = members->Members[0]->GetType()->BaseType;
    ...
    VALUE GetFieldInfo(RubyMemberInfo^ members) {
      FieldInfo^ fieldInfo = (FieldInfo^)members->Members[0];
      members->MemberType  = rb_str_new2("field");
      members->ReturnTypes = CreateSignature(gcnew array<Type^> { fieldInfo->FieldType });
      return members->ToMemberInfo();
    }

    VALUE GetEventInfo(RubyMemberInfo^ members) {
      EventInfo^ eventInfo     = (EventInfo^)members->Members[0];
      MethodInfo^ invokeMethod = eventInfo->EventHandlerType->GetMethod("Invoke");
    ...
    VALUE GetMemberInfo(VALUE camel_case_name, VALUE literal_name, bool isStatic) {
      RubyMemberInfo^ members = GetMembers(camel_case_name, literal_name, isStatic);
      Type^ baseMemberType    = GetMemberType(members);
      ...
      if (_type->IsArray && members->IsIndexer)
        ...
        return GetFieldInfo(members);
      else if (baseMemberType == EventInfo::typeid)
        return GetEventInfo(members);
  ...
  public:
    RubyType(VALUE klass) {
      _type = (Type^)Marshal::ToObject(rb_iv_get(klass, "@clr_type"));
    }
    ...
    VALUE GetInstanceMemberInfo(VALUE camel_case_name, VALUE literal_name) {
      return GetMemberInfo(camel_case_name, literal_name, false);
    }

    VALUE GetStaticMemberInfo(VALUE camel_case_name, VALUE literal_name) {
      return GetMemberInfo(camel_case_name, literal_name, true);
    }
  };
}